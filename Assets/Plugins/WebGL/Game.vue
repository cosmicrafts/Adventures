<template>
  ...
  declare global {
    interface Window {
      ...
      unityInstance?: any; // Add Unity instance for audio context handler
    }
  }
  ...
  const sendAuthDataToUnity = () => {
    ...
    try {
      ...
      // Create ICP identity data object for direct initialization in Unity
      ...
      console.log('Sending auth data to Unity ICPService');
      
      // Send the seed phrase directly to ICPService for initialization
      unityContext.value.sendMessage(
        'ICPService', 
        'InitializeWithSeedPhraseFromJS', 
        seedPhrase
      );
      
      // Also send the full identity data as JSON for more details if needed
      unityContext.value.sendMessage(
        'ICPService', 
        'SetIdentityDataFromJS', 
        JSON.stringify(icpIdentityData)
      );
    ...
    try {
      ...
      unityContext.value
        ...
        .on('mounted', () => {
          ...
          window.unityInstance = unityContext.value; // Make available for audio context handler
          
          // Handle audio context resumption based on our AudioContextResumer.jslib
          document.addEventListener('click', function() {
            if (window.unityInstance && 
                window.unityInstance.Module && 
                window.unityInstance.Module.context && 
                window.unityInstance.Module.context.state !== 'running') {
              window.unityInstance.Module.context.resume().then(function() {
                console.log("AudioContext resumed by page click");
              });
              
              // Also call our custom ResumeAudioContext function
              try {
                // Unity function exposed via AudioContextResumer.jslib
                window.unityInstance.SendMessage('WebGLAudioManager', 'InitializeAudio');
              } catch (e) {
                console.warn("Could not call InitializeAudio:", e);
              }
            }
          }, { once: true });
          
          // Create global function for Unity to call via JavaScriptBridge
          window.dispatchUnityEvent = function(name, ...args) {
            console.log('Unity event dispatched:', name, args);
            
            switch(name) {
              case 'requestAuthData':
                console.log('Unity requested auth data via JavaScriptBridge');
                sendAuthDataToUnity();
                break;
              case 'logoutRequested':
                console.log('Unity requested logout via JavaScriptBridge');
                ...
                break;
              case 'savePlayerData':
                console.log('Saving player data from Unity:', args[0]);
                try {
                  const playerData = JSON.parse(args[0]);
                ...
                break;
              default:
                console.log('Unknown Unity event:', name);
            }
          };
