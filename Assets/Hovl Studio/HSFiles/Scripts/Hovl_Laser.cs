    ...
    public void DisablePrepare()
{
    ...
    if (HitEffect != null)
    {
        HitEffect.SetActive(false); // Ensure HitEffect is hidden
    }
...
{
    // Reset the laser LineRenderer and particle effects
    if (Laser != null)
    {
        Laser.enabled = true;
        Laser.SetPosition(0, transform.position);
        Laser.SetPosition(1, transform.position); // Start with minimal length
    }
